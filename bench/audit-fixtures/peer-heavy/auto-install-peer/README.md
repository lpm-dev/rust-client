# auto-install-peer — Phase 66 R2.2 eager peer fixture

**Tests:** the resolver's R2.2 peer-drain pass auto-installs a
required peer that was absent from the project's `dependencies`.

**Risk:** pre-R2.2, the greedy/fused resolver collected peer
declarations only for the post-resolve `check_unmet_peers` warning
pass. A user installing `react-redux` without manually adding
...
That's the package-manager-as-paperwork failure mode the user
explicitly chose to design out. R2.2 makes peers a first-class
input to the resolver: any required peer not satisfied by the
resolved tree triggers a root-scoped ambient install through the
same dispatcher machinery as regular deps. The package author's
peer declaration becomes a real install obligation, not just
documentation.
...
3. The auto-installed react version satisfies react-redux's peer
   ...
   majors are currently valid post-R2.2 outcomes — react-redux's
...
**Architectural note:** R2.2 deliberately routes the ambient
install through `parent = 0` (root-scope) rather than as a child
edge of the consumer (react-redux). The consumer's
...
- **Post-R2.2 (today):** PASS / PASS in both linker modes.
- **Pre-R2.2 (historical):** FAIL / FAIL — react-redux loads abort
...
even under R2.2's eager default.
