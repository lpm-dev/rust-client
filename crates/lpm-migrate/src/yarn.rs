...
fn parse_dep_line(line: &str) -> Option<(String, String)> {
    ...
    if let Some(stripped) = line.strip_prefix('"') {
        ...
        let end_name = stripped.find('"')?;
        let name = &stripped[..end_name];
        let rest = stripped[end_name + 1..].trim_start();
...
"#;
        ...
        assert_eq!(
            ...
            ("accepts".to_owned(), "1.3.8".to_owned())
        );
    }
...
"#;
        ...
        assert_eq!(
            ...
            ("dep-a".to_owned(), "2.1.0".to_owned())
        );
        assert_eq!(
            my_pkg.dependencies[1],
            ("dep-b".to_owned(), "3.0.1".to_owned())
        );
    }
...
"#;
        ...
        assert!(
            accepts
                .dependencies
                .contains(&("mime-types".to_owned(), "2.1.35".to_owned()))
        );
        assert!(
            accepts
                .dependencies
                .contains(&("negotiator".to_owned(), "0.6.3".to_owned()))
        );
