...
mod tests {
	...
	fn extract_mode_staging() {
		assert_eq!(
			extract_mode_from_env_path(".env.staging"),
			Some("staging")
		);
	}
	...
	fn resolve_from_script_environments_takes_priority_over_alias() {
		...
		envs_map.insert(
			"dev".into(),
			crate::EnvDefinition::File(".env.dev".into()),
		);
	...
	fn sensitive_derived_from_canonical_not_alias() {
		...
		assert!(r.sensitive, "sensitive should derive from canonical 'production', not alias 'release'");
