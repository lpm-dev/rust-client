...
mod tests {
	...
	fn detect_stripe_by_header() {
		...
		assert_eq!(detect_provider("/hook", &headers), Some(WebhookProvider::Stripe));
	}

	#[test]
	fn detect_github_by_header() {
		let headers = make_headers(&[("x-github-event", "push")]);
		assert_eq!(detect_provider("/hook", &headers), Some(WebhookProvider::GitHub));
	}

	#[test]
	fn detect_svix_by_header() {
		let headers = make_headers(&[("svix-id", "msg_123")]);
		assert_eq!(detect_provider("/hook", &headers), Some(WebhookProvider::Svix));
	}
	...
	fn detect_sendgrid_by_header() {
		...
		assert_eq!(detect_provider("/hook", &headers), Some(WebhookProvider::SendGrid));
	}

	#[test]
	fn detect_twilio_by_header() {
		let headers = make_headers(&[("x-twilio-signature", "sig")]);
		assert_eq!(detect_provider("/hook", &headers), Some(WebhookProvider::Twilio));
	}

	#[test]
	fn detect_resend_by_header() {
		let headers = make_headers(&[("x-resend-signature", "sig")]);
		assert_eq!(detect_provider("/hook", &headers), Some(WebhookProvider::Resend));
	}
