...
fn supply_pattern_summary_with_context(
    ...
    let identifiers = patterns[OBF_VAR_NAMES].find_iter(&executable).count();
    SupplyPatternSummary {
        obfuscation: [
            if patterns[OBF_HEX_ESCAPES].is_match(stripped) {
                stripped.matches("\\x").count()
            } else {
                0
            },
            identifiers,
            if patterns[OBF_FROM_CHAR_CODE].is_match(&executable) {
                executable.matches("String.fromCharCode").count()
            } else {
                0
            },
            usize::from(
                context
                    .find_with_context(&patterns[OBF_BUFFER_BASE64], false)
                    .is_some(),
            ),
        ],
        dispatcher: detect_dispatcher_pattern_with_context(stripped, context, identifiers != 0),
...
fn obfuscation_confidence_with_summary(
    ...
    if patterns.obfuscation[OBF_HEX_ESCAPES] != 0 {
        let hex_count = patterns.obfuscation[OBF_HEX_ESCAPES];
    ...
    if patterns.obfuscation[OBF_VAR_NAMES] != 0 {
        let count = patterns.obfuscation[OBF_VAR_NAMES];
    ...
    if patterns.obfuscation[OBF_FROM_CHAR_CODE] != 0 {
        let count = patterns.obfuscation[OBF_FROM_CHAR_CODE];
        let density = count as f64 * per_1k;
    ...
    if patterns.obfuscation[OBF_BUFFER_BASE64] != 0 {
        signal_types += 1;
        score += 0.1;
    }
    ...
    let has_dispatcher = patterns.dispatcher;
    if has_dispatcher {
        signal_types += 1;
        score += 0.5;
    }
...
fn detect_dispatcher_pattern(stripped: &str) -> bool {
    supply_pattern_summary(stripped).dispatcher
}

fn detect_dispatcher_pattern_with_context(
    stripped: &str,
    context: &super::syntax::SourceContext<'_>,
    has_0x_var_names: bool,
) -> bool {
    if !has_0x_var_names {
        return false;
    }
    static ARRAY_ASSIGNMENT: OnceLock<Regex> = OnceLock::new();
    let array = ARRAY_ASSIGNMENT.get_or_init(|| {
        Regex::new(r#"\b_0x[0-9a-f]{4,}\s*=\s*\[\s*["']"#).expect("array assignment regex")
    });
    let has_array = context.find_with_context(array, false).is_some()
    ...
    context.find_with_context(indexed, false).is_some()
}
...
mod tests {
    ...
    #[test]
    fn quoted_decoder_examples_do_not_establish_critical_obfuscation() {
        let code = r#"
            function decode(value) { return String.fromCharCode(value); }
            expect('\\x40', '@');
            expect('var _0xaaaa=["a","b"]', true);
            expect('var _0x1234=["a","b"]', true);
        "#;
        let tags = analyze_supply_chain(code, code.as_bytes());
        assert!(!tags.obfuscated, "{}", tags.obfuscation_confidence);
    }

    #[test]
    fn quoted_dispatcher_examples_do_not_establish_obfuscation() {
        let code =
            r#"const example = "var _0x1234=['a','b']; _0x5678(0x0); String.fromCharCode(42)";"#;
        let tags = analyze_supply_chain(code, code.as_bytes());
        assert!(!tags.obfuscated);
        assert!(!tags.possible_obfuscation);
    }
    ...
    fn supply_patterns_preserve_unicode_word_boundaries() {
        assert_eq!(
            supply_pattern_summary("é_0x1234").obfuscation[OBF_VAR_NAMES],
            0
        );
        assert_eq!(
            supply_pattern_summary("é _0x1234").obfuscation[OBF_VAR_NAMES],
            1
        );
    }
