...
mod tests {
	...
	fn entropy_high_random() {
		...
		assert!(e > 4.0, "random-looking string should have high entropy, got {e}");
	}
	...
	fn entropy_english_text() {
		...
		assert!(e < 4.5, "english text should have moderate entropy, got {e}");
	}
	...
	#[test]
	fn extract_urls() {
		let code = r#"const url = "https://api.example.com/v1"; fetch("https://cdn.example.com/file")"#;
