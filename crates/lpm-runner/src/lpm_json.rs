...
///   the checked-in copy under `a-package-manager/public/schemas/`.
pub fn generate_schema() -> serde_json::Value {
    let mut schema = schemars::schema_for!(LpmJsonConfig);
    let obj = schema.as_object_mut().expect("schema root is an object");
    obj.insert(
        "$id".to_string(),
        serde_json::Value::String("https://lpm.dev/schemas/lpm.json".to_string()),
    );
    obj.insert(
        "title".to_string(),
