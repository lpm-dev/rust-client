...
fn resolve_and_load_env(
	...
	let mode = if let Some(m) = explicit_mode {
		...
		match lpm_json::read_lpm_json(project_dir) {
			Ok(Some(config)) => {
				lpm_json::resolve_env_mode(&config, script_name)
					.and_then(|env_path| {
						lpm_json::extract_mode_from_env_path(&env_path)
							.map(|s| s.to_string())
					})
			}
			_ => None,
...
pub fn list_scripts(
	...
	let mut scripts: Vec<(String, String)> = pkg
		.scripts
		.into_iter()
		.collect();
...
mod tests {
	...
	fn pre_hook_failure_aborts() {
		...
		let result = run_script(dir.path(), "build", &[], None);
		assert!(result.is_err());
		assert!(!marker.exists(), "build should NOT have run after pre-hook failure");
	}
	...
	fn lpm_json_env_mapping() {
		...
		assert!(result.is_ok(), "lpm.json env mapping should auto-load .env.development for 'dev' script");
	}
}
