...
impl WorkspaceGraph {
    ...
    pub fn topological_sort(&self) -> Result<Vec<usize>, GraphError> {
        ...
        for (i, &deps) in unmet_deps.iter().enumerate() {
            if deps == 0 {
                queue.push_back(i);
            }
        }
    ...
    pub fn topological_levels(&self) -> Result<Vec<Vec<usize>>, GraphError> {
        ...
        let mut current_level: Vec<usize> = (0..n).filter(|&i| unmet_deps[i] == 0).collect();
...
mod tests {
    ...
    fn cycle_detected_with_details() {
        ...
        let ws = make_workspace(vec![make_member("a", &["b"]), make_member("b", &["a"])]);
        let graph = WorkspaceGraph::from_workspace(&ws);
