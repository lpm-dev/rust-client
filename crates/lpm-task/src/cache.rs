...
fn restore_archive(archive_path: &Path, project_dir: &Path) -> Result<(), LpmError> {
	...
	for entry in archive.entries().map_err(|e| {
		LpmError::Task(format!("failed to read cache archive entries: {e}"))
	})? {
		let mut entry = entry.map_err(|e| {
			LpmError::Task(format!("failed to read cache archive entry: {e}"))
		})?;
		let path = entry.path().map_err(|e| {
			LpmError::Task(format!("failed to read entry path: {e}"))
		})?.to_path_buf();
		if path
			.components()
		...
		entry.unpack_in(project_dir).map_err(|e| {
