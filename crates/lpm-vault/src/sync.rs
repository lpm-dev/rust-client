...
pub async fn push(
	...
	push_raw(registry_url, auth_token, vault_id, &secrets_json, expected_version, force).await
}
...
pub async fn push_raw(
	...
	if !status.is_success() {
		return Err(result.error.unwrap_or_else(|| format!("server error: {status}")));
	}
...
pub async fn pull(
	...
	let encrypted_blob = result.encrypted_blob
		.ok_or("server returned no encrypted data")?;
	let wrapped_key = result.wrapped_key
...
pub async fn get_my_public_key(
	...
	Ok(data.get("publicKey").and_then(|v| v.as_str()).map(|s| s.to_string()))
}
...
pub async fn pull_org(
	...
	let blob = data.get("encryptedBlob")
		.and_then(|v| v.as_str())
		.ok_or("no encryptedBlob in response")?;
	let wrapped = data.get("wrappedKey")
		.and_then(|v| v.as_str())
