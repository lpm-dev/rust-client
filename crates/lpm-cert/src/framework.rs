...
fn get_framework_env(
	...
	match framework {
		...
		Framework::Vite => vec![
			...
			("VITE_DEV_SERVER_HTTPS_CERT".to_string(), cert_path.to_string()),
			("VITE_DEV_SERVER_HTTPS_KEY".to_string(), key_path.to_string()),
		],
		...
		Framework::Nuxt => vec![
			...
			("NUXT_DEVSERVER_HTTPS_CERT".to_string(), cert_path.to_string()),
