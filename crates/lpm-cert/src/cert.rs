...
pub fn needs_renewal(cert_path: &Path) -> Result<bool, LpmError> {
	...
	let pem = pem::parse(&pem_str)
		.map_err(|e| LpmError::Cert(format!("invalid PEM: {e}")))?;
...
mod tests {
	...
	fn project_cert_has_default_sans() {
		...
		assert!(san_strs.iter().any(|s| s.contains("localhost")), "missing localhost in SANs: {san_strs:?}");
		// IP addresses are shown in hex by x509-parser: 127.0.0.1 = 7f:00:00:01
		assert!(san_strs.iter().any(|s| s.contains("7f:00:00:01")), "missing 127.0.0.1 in SANs: {san_strs:?}");
	}
	...
	fn project_cert_with_extra_hostnames() {
		...
		assert!(san_strs.iter().any(|s| s.contains("myapp.test")), "missing custom host: {san_strs:?}");
		// 192.168.1.42 in hex = c0:a8:01:2a
		assert!(san_strs.iter().any(|s| s.contains("c0:a8:01:2a")), "missing custom IP: {san_strs:?}");
	}
