...
//! coverage.
//!
//! Locks the contract emitted by `lpm setup ci gitlab` against the resolver
//! that backs `lpm setup ci npmrc --oidc`.
//!
//! The CLI's GitLab snippet generator (`crates/lpm-cli/src/commands/setup.rs`)
...
//! `LPM_OIDC_TOKEN`, and confirms `lpm setup ci npmrc --oidc` POSTs that JWT to the
...
async fn gitlab_snippet_lpm_oidc_token_drives_setup_oidc() {
    ...
    assert!(
        ...
        "lpm setup ci npmrc --oidc must succeed when LPM_OIDC_TOKEN is set\n  exit: {:?}\n  stdout:\n{}\n  stderr:\n{}",
        output.status.code(),
    ...
    assert!(
        ...
        ".npmrc must carry the exchanged session token (locks the wire format \
         emitted by `lpm setup ci gitlab`):\n{npmrc}"
    );
    assert!(
