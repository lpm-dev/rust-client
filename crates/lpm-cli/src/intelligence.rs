...
pub fn detect_phantom_deps(
	...
	let builtins: HashSet<&str> = [
		...
		"v8", "vm", "zlib", "assert", "console", "constants", "module", "process",
		"punycode", "timers", "worker_threads", "perf_hooks", "async_hooks",
		"diagnostics_channel",
		// Node: prefixed
		"node:fs", "node:path", "node:os", "node:url", "node:http", "node:https",
		"node:crypto", "node:stream", "node:util", "node:events", "node:buffer",
		"node:child_process", "node:net", "node:test", "node:assert",
	]
	.into_iter()
	.collect();
...
pub fn verify_imports(
	...
	let builtins: HashSet<&str> = [
		"fs", "path", "os", "url", "http", "https", "crypto", "stream", "util",
		"events", "buffer", "child_process", "net", "querystring", "readline",
		"assert", "console", "module", "process", "timers", "worker_threads",
		"perf_hooks", "async_hooks", "diagnostics_channel", "node:fs", "node:path",
	...
	for import in &imports {
		...
		if !installed_packages.contains(pkg_name) && !seen_packages.contains(pkg_name) {
			...
			let suggestion = if declared_deps.contains_key(pkg_name) {
				format!("Declared but not installed. Run: lpm-rs install")
			} else {
