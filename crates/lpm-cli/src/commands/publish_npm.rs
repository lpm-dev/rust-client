...
fn validate_npm_name(name: &str) -> Result<(), LpmError> {
	if name.is_empty() {
		return Err(LpmError::Registry("npm package name cannot be empty".into()));
	}
	if name.len() > 214 {
		return Err(LpmError::Registry(
			format!("npm package name too long ({} chars, max 214)", name.len()),
		));
	}
	...
	if check_name != check_name.to_lowercase() {
		return Err(LpmError::Registry(
			format!("npm package name must be lowercase: \"{name}\""),
		));
	}
	...
	for ch in check_name.chars() {
		if !ch.is_ascii_alphanumeric() && !"-._~".contains(ch) {
			return Err(LpmError::Registry(
				format!("npm package name contains invalid character '{ch}': \"{name}\""),
			));
		}
	}
...
pub fn resolve_npm_access(
	...
	// npm default: scoped packages are restricted, unscoped are public
	if npm_name.starts_with('@') {
		"public".to_string() // We default scoped to public (npm requires explicit for first publish)
	} else {
		"public".to_string()
	}
}
...
pub async fn publish_to_npm(
	...
	let payload = build_npm_payload(npm_name, version, version_data, tarball_data, access, Some(tag));
	...
	let response = req.send().await.map_err(|e| {
		LpmError::Registry(format!("npm publish request failed: {e}"))
	})?;
	...
	if status == reqwest::StatusCode::UNAUTHORIZED {
		...
		if needs_otp {
			...
			let retry_response = retry_req.send().await.map_err(|e| {
				LpmError::Registry(format!("npm publish retry failed: {e}"))
			})?;
...
async fn handle_npm_response(
	...
	let detailed_error = match status.as_u16() {
		401 => format!(
			"authentication failed — check token permissions. Run `lpm login --npm`"
		),
		404 => format!(
			"not found — this usually means the auth token is missing or invalid. Run `lpm login --npm`"
		),
		402 => format!(
			"npm requires a paid plan for private packages. Publish with `access: \"public\"` or upgrade your npm plan."
		),
		...
		429 => format!(
			"npm rate limit exceeded. Wait and try again."
		),
		400 => format!(
...
mod tests {
	...
	fn resolve_npm_access_with_config() {
		...
		assert_eq!(resolve_npm_access("@scope/pkg", Some(&config)), "restricted");
	}
	...
	fn otp_header_detection() {
		...
		assert!(!is_otp_required(reqwest::StatusCode::UNAUTHORIZED, &empty_headers));
