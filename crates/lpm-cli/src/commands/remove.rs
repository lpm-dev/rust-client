...
mod windows_directory_security {
    ...
    fn directory_rollback_preserves_protected_and_unprotected_dacls() {
        for protected in [false, true] {
            ...
            apply_sddl(handle.as_raw_handle(), "D:(A;OICI;GR;;;WD)(A;OICI;FA;;;OW)(A;OICI;FA;;;SY)", protected).unwrap();
            let expected = snapshot(&original).unwrap();
            assert_eq!(expected.protected, protected);
            assert!(!expected.sddl.contains('\0'));
            let (private_name, restored) = crate::directory_transaction::create_private_directory(&parent, "restore-test").unwrap();
            restore(&restored, &expected).unwrap();
            crate::directory_transaction::publish_directory_noreplace(&parent, &restored, &private_name, &parent, std::ffi::OsStr::new("restored")).unwrap();
