...
pub async fn run(
	...
	let behavioral_results = run_behavioral_analysis(
		&discovery,
		&mut results,
		&lpm_packages,
		json_output,
	...
	// ── Exit code: non-zero when vulnerabilities or critical behaviors found ──
	...
	let has_registry_vulns = results.iter().any(|r| {
		r.issues
			.iter()
			.any(|i| i.category == "vulnerability")
	});

	if has_vulns || has_critical_behavior || has_registry_vulns {
...
fn run_behavioral_analysis(
	...
	let scannable: Vec<&DiscoveredPackage> = discovery
		.packages
		.iter()
		.filter(|p| matches!(p.scan_mode, ScanMode::FullLocal | ScanMode::RegistryAndStore))
		.collect();
	...
	for (i, pkg) in scannable.iter().enumerate() {
		...
		let analysis = analysis.or_else(|| {
			...
			if abs_path.is_dir() {
				...
				if project_cache.is_none() {
					project_cache = Some(ProjectAuditCache::new(
						&discovery.manager.to_string(),
					));
				}
		...
		if let Some(&idx) = results_by_key.get(&merge_key) {
			...
			let existing_messages: HashSet<String> =
				results[idx].issues.iter().map(|i| i.message.clone()).collect();
			for issue in issues {
...
fn analysis_to_issues(analysis: &lpm_security::behavioral::PackageAnalysis, source: &str) -> Vec<AuditIssue> {
...
fn print_discovery_header(discovery: &DiscoveryResult, lpm_count: usize, npm_count: usize) {
	...
	if let Some(ref lockfile_path) = discovery.lockfile_path {
		...
		println!(
			"  {} {}",
			"│".dimmed(),
			"No lockfile found — scanning node_modules directly (degraded mode)"
				.yellow(),
		);
	}
	...
	if discovery.is_yarn_pnp {
		println!(
			"  {} {}",
			"│".dimmed(),
			"Yarn PnP detected — source scanning unavailable (packages are zipped)"
				.yellow(),
		);
		println!(
...
fn print_summary(
	...
	if vuln_count == 0 && lpm_issues == 0 && behavioral.packages_with_findings == 0 {
		...
		if vuln_count > 0 {
			parts.push(format!("{vuln_count} vulnerabilit{}", if vuln_count == 1 { "y" } else { "ies" }));
		}
		if behavioral.packages_with_findings > 0 {
			parts.push(format!("{} suspicious", behavioral.packages_with_findings));
		}
		if lpm_issues > 0 {
			parts.push(format!("{lpm_issues} LPM issue{}", if lpm_issues == 1 { "" } else { "s" }));
		}
