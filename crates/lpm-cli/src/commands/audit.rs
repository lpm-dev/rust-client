...
pub async fn run(
    ...
    if combined_issues == 0 {
        ...
        if total_osv > 0 {
            output::warn(&format!("{total_osv} known vulnerabilities found via OSV"));
        }
        if packages_with_issues == 0 && total_osv == 0 {
            output::success(&format!("No issues found ({checked} packages audited)"));
        }
    }
    println!();
...
fn collect_all_packages(project_dir: &Path) -> Vec<(String, String)> {
    ...
    if lockfile_path.exists()
        && let Ok(lockfile) = lpm_lockfile::Lockfile::read_fast(&lockfile_path)
    {
        return lockfile
            .packages
            .iter()
...
fn collect_lpm_packages(project_dir: &Path) -> Result<Vec<(String, String)>, LpmError> {
    ...
    if pkg_json_path.exists() {
        let content = std::fs::read_to_string(&pkg_json_path).map_err(LpmError::Io)?;
    ...
    if package_swift.exists() {
        let content = std::fs::read_to_string(&package_swift).map_err(LpmError::Io)?;
