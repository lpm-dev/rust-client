...
pub async fn run(json_output: bool) -> Result<(), LpmError> {
	...
	if let Some(s) = spinner {
		s.stop(format!("Current: {}  Latest: {}", current.dimmed(), latest.bold()));
	}
	...
	if latest == current {
		if json_output {
			...
			output::success(&format!("Already on the latest version ({})", current.bold()));
		}
		return Ok(());
	}
	...
	match method {
		InstallMethod::Npm => run_shell_update("npm", &["install", "-g", "@lpm-registry/cli@latest"])?,
		InstallMethod::Homebrew => run_shell_update("brew", &["upgrade", "lpm"])?,
		InstallMethod::Cargo => {
			run_shell_update(
				"cargo",
				&["install", "--git", "https://github.com/lpm-dev/rust-client", "lpm-cli", "--force"],
			)?;
		}
...
fn detect_install_method() -> InstallMethod {
	...
	if exe_path.contains("homebrew") || exe_path.contains("Cellar") || exe_path.contains("linuxbrew") {
		...
		InstallMethod::Cargo
	} else if exe_path.contains("node_modules") || exe_path.contains("npm") || exe_path.contains("nvm") {
		InstallMethod::Npm
	} else {
		InstallMethod::Standalone
	}
...
async fn run_standalone_update(version: &str) -> Result<(), LpmError> {
	...
	spinner.stop(format!("Downloaded {} ({})", binary_name, format_bytes(bytes.len())));
	...
	std::fs::write(&tmp_path, &bytes)
		.map_err(|e| LpmError::Io(std::io::Error::new(e.kind(), format!("failed to write temp binary: {e}"))))?;
	...
	std::fs::rename(&tmp_path, &current_exe)
		.map_err(|e| LpmError::Io(std::io::Error::new(e.kind(), format!("failed to replace binary: {e}"))))?;
...
mod tests {
	...
	fn detect_platform_returns_valid_tuple() {
		...
		assert!(result.is_ok(), "detect_platform should succeed on this host");
