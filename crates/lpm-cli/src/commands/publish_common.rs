    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        if path.is_dir() {
            if dir == project_root && name_str == ".lpm" {
                collect_implicit_lpm_skills(&path, project_root, canonical_root, result)?;
                continue;
            }
...
fn collect_implicit_lpm_skills(
    lpm_dir: &Path,
    project_root: &Path,
    canonical_root: &Path,
    result: &mut Vec<TarballFile>,
    ...
    let report = crate::commands::skills::author::validate_directory(&skills_dir)?;
    for relative in report.valid_files {
        let path = skills_dir.join(&relative);
        if !is_safe_entry(&path, canonical_root)
            || is_materialized_package_skill_path(&path, project_root)
        {
            continue;
        }
        let metadata = std::fs::symlink_metadata(&path)?;
        result.push(TarballFile {
            path: format!(".lpm/skills/{relative}"),
            size: metadata.len(),
        });
    }
    Ok(())
