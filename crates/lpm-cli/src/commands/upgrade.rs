...
fn compute_upgrade(
	...
	let range_body = current_range.trim_start_matches(|c: char| c == '^' || c == '~');
...
mod tests {
	...
	fn default_mode_skips_prereleases() {
		let available = vec!["1.2.0".into(), "1.6.0-beta.1".into(), "1.5.0".into(), "2.0.0".into()];
	...
	fn string_replacement_preserves_tab_formatting() {
		...
		assert!(!updated.contains("\"^1.2.0\""), "old version should be gone");
	}
	...
	fn extract_deps_from_json_value() {
		...
		assert!(deps.iter().any(|(n, r, d)| n == "foo" && r == "^1.0.0" && !d));
		assert!(deps.iter().any(|(n, r, d)| n == "bar" && r == "~2.0.0" && *d));
	}
}
