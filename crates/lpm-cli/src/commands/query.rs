...
use lpm_security::query::{
	self, count_all_tags, parse_selector, DepGraph, PackageContext, Severity,
};
...
pub async fn run(
	...
	let selector_str = selector_str.ok_or_else(|| {
		LpmError::Registry("No selector provided. Usage: lpm query \":eval\" or lpm query --count".into())
	})?;

	let selector = parse_selector(selector_str).map_err(|e| {
		LpmError::Registry(format!("Invalid selector: {e}"))
	})?;
	...
	if json_output {
		...
		println!("  {} No packages match {}", "·".dimmed(), selector_str.bold());
	} else {
		println!(
			"  {} {} matching {}",
			"●".green(),
			format!("{} package{}", matched.len(), if matched.len() == 1 { "" } else { "s" }).bold(),
			selector_str.bold()
		);
		println!();
...
fn run_count_mode(packages: &[PackageContext<'_>], json_output: bool) -> Result<(), LpmError> {
	...
	if json_output {
		...
		for tc in &counts {
			json_obj.insert(
				tc.pseudo_class.display_name().trim_start_matches(':').to_string(),
	...
	let tiers: [(Severity, &str, fn(&str) -> String); 4] = [
		(Severity::Critical, "Critical", |s: &str| s.red().bold().to_string()),
		(Severity::High, "High", |s: &str| s.yellow().bold().to_string()),
		(Severity::Medium, "Medium", |s: &str| s.cyan().bold().to_string()),
...
fn collect_active_tags(analysis: &PackageAnalysis) -> Vec<String> {
	...
	// Source tags
	if analysis.source.eval { tags.push("eval".into()); }
	if analysis.source.network { tags.push("network".into()); }
	if analysis.source.filesystem { tags.push("fs".into()); }
	if analysis.source.shell { tags.push("shell".into()); }
	if analysis.source.child_process { tags.push("child-process".into()); }
	if analysis.source.native_bindings { tags.push("native".into()); }
	if analysis.source.crypto { tags.push("crypto".into()); }
	if analysis.source.dynamic_require { tags.push("dynamic-require".into()); }
	if analysis.source.environment_vars { tags.push("env".into()); }
	if analysis.source.web_socket { tags.push("ws".into()); }

	// Supply chain tags
	if analysis.supply_chain.obfuscated { tags.push("obfuscated".into()); }
	if analysis.supply_chain.high_entropy_strings { tags.push("high-entropy".into()); }
	if analysis.supply_chain.minified { tags.push("minified".into()); }
	if analysis.supply_chain.telemetry { tags.push("telemetry".into()); }
	if analysis.supply_chain.url_strings { tags.push("url-strings".into()); }
	if analysis.supply_chain.trivial { tags.push("trivial".into()); }
	if analysis.supply_chain.protestware { tags.push("protestware".into()); }

	// Manifest tags
	if analysis.manifest.git_dependency { tags.push("git-dep".into()); }
	if analysis.manifest.http_dependency { tags.push("http-dep".into()); }
	if analysis.manifest.wildcard_dependency { tags.push("wildcard-dep".into()); }
	if analysis.manifest.copyleft_license { tags.push("copyleft".into()); }
	if analysis.manifest.no_license { tags.push("no-license".into()); }
...
fn check_has_lifecycle_scripts(pkg_dir: &Path) -> bool {
	...
	LIFECYCLE_SCRIPTS.iter().any(|phase| scripts.contains_key(*phase))
}
