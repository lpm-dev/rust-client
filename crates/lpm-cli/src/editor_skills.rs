        ...
        match editor {
            ...
            AiEditor::GitHubCopilot => {
                if append_skills_reference(project_dir, ".github/copilot-instructions.md") {
                    integrated.push(format!("{}: added skills reference", editor.name()));
                }
            }
            AiEditor::Augment => {
...
fn symlink_skills_to_cursor(project_dir: &Path, skills_dir: &Path) -> std::io::Result<usize> {
    ...
    for pkg_entry in std::fs::read_dir(skills_dir)?.flatten() {
        ...
        for skill_entry in std::fs::read_dir(pkg_entry.path())?.flatten() {
            ...
            if skill_path.extension().map(|e| e == "md").unwrap_or(false) {
                ...
                let safe_pkg = lpm_common::sanitize_path_component(&pkg_name.to_string_lossy());
                // Unique name: {pkg}--{skill}.md
                let link_name = format!("{}--{}", safe_pkg, skill_name.to_string_lossy());
...
mod tests {
    ...
    fn remove_editor_skills_cleans_cursor_symlinks() {
        ...
        fs::write(cursor_rules.join("owner.pkg--guide.md"), "skill content").unwrap();
        fs::write(cursor_rules.join("owner.pkg--api.md"), "skill content").unwrap();
        fs::write(cursor_rules.join("other.pkg--guide.md"), "other content").unwrap();
