...
fn workspace_protocol_resolution() {
	...
	let ws = lpm_workspace::discover_workspace(&path)
		.unwrap()
		.unwrap();
...
fn task_graph_respects_depends_on() {
	...
	let tasks: HashMap<String, lpm_runner::lpm_json::TaskConfig> = HashMap::from([
		("ci".to_string(), ci_task),
		("test".to_string(), test_task),
	]);
...
fn linker_isolated_mode_creates_lpm_dir() {
	...
	assert!(project_dir
		.join("node_modules/.lpm/ms@2.1.3")
		.exists());
...
fn linker_hoisted_mode_flattens() {
	...
	let result =
		lpm_linker::link_packages_hoisted(&project_dir, &targets, false).unwrap();
