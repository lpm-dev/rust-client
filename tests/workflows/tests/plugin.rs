...
const ROLLDOWN_UPDATE_VERSION: &str = "1.1.4";
...
async fn mount_rolldown_update_graph(server: &MockServer) {
    let root_tarball = create_test_tarball(&[
        ...
        (
            "package.json",
            br#"{"name":"rolldown","version":"1.1.4","bin":{"rolldown":"./bin/cli.mjs"}}"#,
        ),
    ]);
    ...
    let oxc_types_tarball = create_test_tarball(&[(
        "package.json",
        br#"{"name":"@oxc-project/types","version":"0.137.0"}"#,
    )]);
    ...
    let root_path = "/rolldown/-/rolldown-1.1.4.tgz";
    let pluginutils_path = "/@rolldown/pluginutils/-/pluginutils-1.1.0.tgz";
    let oxc_types_path = "/@oxc-project/types/-/types-0.137.0.tgz";
...
fn plugin_list_json_reports_installed_versions_and_known_latest_versions() {
    ...
    assert_eq!(oxlint["latest"], serde_json::json!("1.71.0"));
    ...
    assert_eq!(biome["latest"], serde_json::json!("2.5.1"));
    ...
    assert_eq!(rolldown["latest"], serde_json::json!("1.1.3"));
...
fn plugin_list_human_renders_table_and_slim_completion() {
    let project = TempProject::empty(r#"{"name":"plugin-test","version":"1.0.0"}"#);
    seed_installed_plugin(&project, "oxlint", "1.57.0");
    seed_installed_plugin(&project, "biome", "2.5.1");
    ...
    assert!(
        stdout_raw.contains("\u{1b}[2mPlugin")
            ...
            && stdout_raw.contains("\u{1b}[33m1.71.0")
...
async fn plugin_outdated_json_reports_managed_tools_and_project_owned_tsdown() {
    ...
    assert_eq!(plugin_entry(plugins, "oxlint")["latest"], "1.72.0");
    assert_eq!(plugin_entry(plugins, "biome")["latest"], "2.5.2");
    assert_eq!(
