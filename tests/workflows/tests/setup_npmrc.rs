...
async fn setup_npmrc_json_writes_scoped_config_gitignore_and_read_only_token() {
    ...
    assert!(
        npmrc.contains("# LPM Registry (generated by lpm setup-npmrc — do not commit)"),
...
#[tokio::test]
async fn setup_npmrc_non_json_output_uses_current_command_name() {
    let project = TempProject::empty(r#"{"name":"web app","version":"1.0.0"}"#);
    let mock = MockRegistry::start().await;
    ...
    let output = lpm_with_registry(&project, &mock.url())
        .arg("setup-npmrc")
        .output()
        .expect("failed to run lpm setup-npmrc");

    assert!(
        output.status.success(),
        "lpm setup-npmrc failed:\nstdout: {}\nstderr: {}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr),
    );
    ...
    assert!(
        combined.contains("Run `lpm setup-npmrc` again to refresh when expired."),
