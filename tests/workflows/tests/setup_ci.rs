...
#[test]
fn setup_ci_reports_environment_bearers_in_oidc_capable_ci() {
    let project = TempProject::empty(r#"{"name":"ci-source"}"#);
    let output = lpm(&project)
        .env("LPM_TOKEN", "ci-bearer")
        .env("CI", "true")
        .env("CI_JOB_JWT_V2", "provider-identity")
        .args(["setup", "ci", "npmrc", "--json"])
        .output()
        .unwrap();
    assert!(output.status.success());
    let result: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
    assert_eq!(result["uses_env_var"], true);
}

#[tokio::test]
async fn setup_ci_does_not_overwrite_npmrc_edits_during_retirement() {
    let project = TempProject::empty(r#"{"name":"setup-concurrent"}"#);
    let mock = MockRegistry::start().await;
    project.write_file(".npmrc", &format!(
        "# LPM Registry (generated by lpm setup local — do not commit)\n@lpm.dev:registry={}/api/registry\n{}/api/registry/:_authToken=previous-token\n# End LPM Registry\n",
        mock.url(), mock.url().replace("http:", "")
    ));
    let npmrc_path = project.path().join(".npmrc");
    wiremock::Mock::given(wiremock::matchers::method("POST"))
        .and(wiremock::matchers::path("/api/registry/-/token/revoke-project"))
        .respond_with(move |_: &wiremock::Request| {
            ...
            wiremock::ResponseTemplate::new(200).set_body_json(serde_json::json!({"revoked":true,"tokenId":"11111111-1111-4111-8111-111111111111"}))
        })
        .expect(1)
        .mount(mock.server())
        .await;
    let output = lpm_with_registry(&project, &mock.url())
        .env("LPM_TOKEN", "new-bearer")
        .args(["setup", "ci", "npmrc"])
        .output()
        .unwrap();
    assert!(
